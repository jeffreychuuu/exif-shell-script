...
| **3. 強制指定相簿**<br>（全數塞入特定相簿） | **`自訂相簿名稱`** | `SourceFolder: "/.../實體路徑"`<br>`Album: "name:自訂相簿名稱"`     | 忽略任何子資料夾結構，強制將該路徑下所有相片打包上傳至雲端指定名稱的單一相簿中。<br>⚠️ `film-exif-installer.sh` 支援含空格的名稱直接輸入，無需用 `""` 包住。 |
